	...
	#define V_COMMIT_HASH "d1f57ea"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "d1f57ea"
#endif
...
typedef struct libmus__PitchClass libmus__PitchClass;
...
typedef struct _result_libmus__PitchClass _result_libmus__PitchClass;
...
struct _result_libmus__PitchClass {
	bool is_error;
	IError err;
	byte data[sizeof(libmus__PitchClass) > 1 ? sizeof(libmus__PitchClass) : 1];
};
...
VV_LOCAL_SYMBOL  int compare_494480560885275829_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VV_LOCAL_SYMBOL int libmus__PitchClass_get_step(libmus__PitchClass pc);
VV_LOCAL_SYMBOL void libmus__PitchClass_set_step(libmus__PitchClass* pc, int value);
VV_LOCAL_SYMBOL int libmus__PitchClass_get_alteration(libmus__PitchClass pc);
VV_LOCAL_SYMBOL void libmus__PitchClass_set_alteration(libmus__PitchClass* pc, int value);
VV_LOCAL_SYMBOL _result_libmus__PitchClass libmus__random_pitchclass(void);
VV_LOCAL_SYMBOL _result_libmus__PitchClass libmus__random_pitchclass_extended(void);
VV_LOCAL_SYMBOL string libmus__PitchClass_name(libmus__PitchClass pc);
string libmus__PitchClass_pretty_name(libmus__PitchClass pc);
VV_LOCAL_SYMBOL int libmus__truncate_range(int value, int min, int max);
...
string _const_libmus__flat_symbol; // a string literal, inited later
string _const_libmus__sharp_symbol; // a string literal, inited later
string _const_libmus__natural_symbol; // a string literal, inited later
string _const_libmus__double_flat_symbol; // a string literal, inited later
string _const_libmus__double_sharp_symbol; // a string literal, inited later
#define _const_libmus__pitchclass_min_step 1
#define _const_libmus__pitchclass_max_step 7
#define _const_libmus__pitchclass_min_alteration -2
#define _const_libmus__pitchclass_max_alteration 2
...
Array_string _const_libmus__pitchclass_names; // inited later
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	...
	if (sidx == _IError_time__TimeParseError_index) return 130;
	return 29;
}
...
static int v_typeof_interface_idx_rand__PRNG(int sidx) { /* rand.PRNG */ 
	if (sidx == _rand__PRNG_rand__wyrand__WyRandRNG_index) return 65655;
...
string string_replace_each(string s, Array_string vals) {
	...
	qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_494480560885275829_RepIndex_by_idx);
	u8* b = malloc_noscan(new_len + 1);
	int idx_pos = 0;
...
VV_LOCAL_SYMBOL void libmus__PitchClass_set_step(libmus__PitchClass* pc, int value) {
	if (value < _const_libmus__pitchclass_min_step) {
		return;
	}
	pc->step = (value - _const_libmus__pitchclass_min_step) % (_const_libmus__pitchclass_max_step - _const_libmus__pitchclass_min_step + 1) + _const_libmus__pitchclass_min_step;
}

VV_LOCAL_SYMBOL int libmus__PitchClass_get_alteration(libmus__PitchClass pc) {
	int _t1 = pc.alteration;
	return _t1;
}

VV_LOCAL_SYMBOL void libmus__PitchClass_set_alteration(libmus__PitchClass* pc, int value) {
	pc->alteration = libmus__truncate_range(value, _const_libmus__pitchclass_min_alteration, _const_libmus__pitchclass_max_alteration);
}

VV_LOCAL_SYMBOL _result_libmus__PitchClass libmus__random_pitchclass(void) {
	_result_int _t2 = rand__int_in_range(_const_libmus__pitchclass_min_step, _const_libmus__pitchclass_max_step + 1);
	if (_t2.is_error) {
		_result_libmus__PitchClass _t3;
		memcpy(&_t3, &_t2, sizeof(_result));
		return _t3;
	}
	_result_int _t4 = rand__int_in_range(-1, 2);
	if (_t4.is_error) {
		_result_libmus__PitchClass _t5;
		memcpy(&_t5, &_t4, sizeof(_result));
		return _t5;
	}
	
 	
 	_result_libmus__PitchClass _t1;
	_result_ok(&(libmus__PitchClass[]) { ((libmus__PitchClass){.step =  (*(int*)_t2.data),.alteration =  (*(int*)_t4.data),}) }, (_result*)(&_t1), sizeof(libmus__PitchClass));
	return _t1;
}
...
VV_LOCAL_SYMBOL _result_libmus__PitchClass libmus__random_pitchclass_extended(void) {
	...
	_result_int _t4 = rand__int_in_range(_const_libmus__pitchclass_min_alteration, _const_libmus__pitchclass_max_alteration + 1);
	if (_t4.is_error) {
		_result_libmus__PitchClass _t5;
		memcpy(&_t5, &_t4, sizeof(_result));
		return _t5;
...
VV_LOCAL_SYMBOL string libmus__PitchClass_name(libmus__PitchClass pc) {
	string result = (*(string*)array_get(_const_libmus__pitchclass_names, pc.step - 1));
	for (int _t1 = 1; _t1 < pc.alteration + 1; ++_t1) {
		result = string__plus(result, _SLIT("#"));
	}
	for (int _t2 = pc.alteration; _t2 < 0; ++_t2) {
		result = string__plus(result, _SLIT("b"));
	}
	return result;
}

string libmus__PitchClass_pretty_name(libmus__PitchClass pc) {
	string name = (*(string*)array_get(_const_libmus__pitchclass_names, pc.step - 1));
	string _t1 = ((pc.alteration == (-2))? (string__plus(name, _const_libmus__double_flat_symbol)) : (pc.alteration == (-1))? (string__plus(name, _const_libmus__flat_symbol)) : (pc.alteration == (1))? (string__plus(name, _const_libmus__sharp_symbol)) : (pc.alteration == (2))? (string__plus(name, _const_libmus__double_sharp_symbol)) : (name));
	return _t1;
}
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	rand__init();
	// Initializations for module libmus
	_const_libmus__flat_symbol = _SLIT("\342\231\255");
	_const_libmus__sharp_symbol = _SLIT("\342\231\257");
	_const_libmus__natural_symbol = _SLIT("\342\231\256");
	_const_libmus__double_flat_symbol = _SLIT("\360\235\204\253");
	_const_libmus__double_sharp_symbol = _SLIT("\360\235\204\252");
	_const_libmus__pitchclass_names = new_array_from_c_array(7, 7, sizeof(string), _MOV((string[7]){_SLIT("C"), _SLIT("D"), _SLIT("E"), _SLIT("F"), _SLIT("G"), _SLIT("A"), _SLIT("B")}));
}
void _vcleanup(void) {
}
